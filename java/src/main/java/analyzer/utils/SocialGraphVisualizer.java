...
public class SocialGraphVisualizer {
    ...
    public static String generateTextVisualization(Collection<AuthorData> authorDataCollection) {
        ...
        sb.append("- Total Mentions: ").append(stats.getTotalMentions()).append("\n");
        sb.append("- Total Connections: ").append(stats.getTotalConnections()).append("\n");
        sb.append("- Avg Connections/User: ").append(String.format("%.2f", stats.getAverageConnectionsPerUser())).append("\n");
        sb.append("- Avg Mentions/User: ").append(String.format("%.2f", stats.getAverageMentionsPerUser())).append("\n\n");
    ...
    public static String generateHTMLContent(Collection<AuthorData> authorDataCollection) {
        ...
        html.append("<p><strong>Total Users:</strong> ").append(stats.getTotalUsers()).append("</p>\n");
        html.append("<p><strong>Total Mentions:</strong> ").append(stats.getTotalMentions()).append("</p>\n");
        html.append("<p><strong>Total Connections:</strong> ").append(stats.getTotalConnections()).append("</p>\n");
        html.append("<p><strong>Average Connections per User:</strong> ").append(String.format("%.2f", stats.getAverageConnectionsPerUser())).append("</p>\n");
        html.append("</div>\n");
        ...
        for (int i = 0; i < graphData.getNodes().size(); i++) {
            Node node = graphData.getNodes().get(i);
            html.append("  {id: '").append(node.getId()).append("', ");
            html.append("label: '").append(escapeJs(node.getLabel())).append("', ");
            html.append("value: ").append(node.getMentionsSent() + node.getMentionsReceived()).append(", ");
            html.append("title: 'Sent: ").append(node.getMentionsSent()).append(", Received: ").append(node.getMentionsReceived()).append("'}");
            if (i < graphData.getNodes().size() - 1) html.append(",");
            html.append("\n");
        }
        ...
        // Add edges
        for (int i = 0; i < graphData.getEdges().size(); i++) {
            Edge edge = graphData.getEdges().get(i);
            html.append("  {from: '").append(edge.getSource()).append("', ");
            html.append("to: '").append(edge.getTarget()).append("', ");
            html.append("value: ").append(edge.getWeight()).append(", ");
            html.append("title: '").append(edge.getWeight()).append(" mentions', ");
            html.append("arrows: 'to'}");
            if (i < graphData.getEdges().size() - 1) html.append(",");
            html.append("\n");
        }
    ...
    public static void exportGephiFormat(Collection<AuthorData> authorDataCollection, String outputPath) throws IOException {
        ...
        gexf.append("<meta lastmodifieddate=\"").append(new Date().toString()).append("\">\n");
        ...
        for (Node node : graphData.getNodes()) {
            gexf.append("<node id=\"").append(node.getId()).append("\" label=\"").append(escapeXml(node.getLabel())).append("\">\n");
            gexf.append("<attvalues>\n");
            gexf.append("<attvalue for=\"0\" value=\"").append(node.getMentionsSent()).append("\"/>\n");
            gexf.append("<attvalue for=\"1\" value=\"").append(node.getMentionsReceived()).append("\"/>\n");
            gexf.append("</attvalues>\n");
            gexf.append("</node>\n");
        }
        gexf.append("</nodes>\n");
        ...
        for (Edge edge : graphData.getEdges()) {
            gexf.append("<edge id=\"").append(edgeId++).append("\" source=\"").append(edge.getSource())
                .append("\" target=\"").append(edge.getTarget()).append("\" weight=\"").append(edge.getWeight()).append("\"/>\n");
        }
        gexf.append("</edges>\n");
    ...
    public static void exportGraphMLFormat(Collection<AuthorData> authorDataCollection, String outputPath) throws IOException {
        ...
        // Nodes
        for (Node node : graphData.getNodes()) {
            graphML.append("<node id=\"").append(node.getId()).append("\">\n");
            graphML.append("<data key=\"label\">").append(escapeXml(node.getLabel())).append("</data>\n");
            graphML.append("<data key=\"mentionsSent\">").append(node.getMentionsSent()).append("</data>\n");
            graphML.append("<data key=\"mentionsReceived\">").append(node.getMentionsReceived()).append("</data>\n");
            graphML.append("</node>\n");
        }
        ...
        int edgeId = 0;
        for (Edge edge : graphData.getEdges()) {
            graphML.append("<edge id=\"e").append(edgeId++).append("\" source=\"").append(edge.getSource())
                .append("\" target=\"").append(edge.getTarget()).append("\">\n");
            graphML.append("<data key=\"weight\">").append(edge.getWeight()).append("</data>\n");
            graphML.append("</edge>\n");
        }
